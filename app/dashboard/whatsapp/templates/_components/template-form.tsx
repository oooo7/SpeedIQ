...
// to numbered {{1}}, {{2}} on save for Meta, and back to friendly names on load.
// ---------------------------------------------------------------------------
...
function numberedBodyToFriendly(
  text: string,
  mapping: string[] | null | undefined
): string {
  if (!text || !Array.isArray(mapping) || mapping.length === 0) return text;
  let result = text;
  mapping.forEach((key, i) => {
    if (!key) return;
    // mapping may be "first_name" or "custom:order_id" — strip prefix for placeholder name
    const friendly = key.startsWith("custom:") ? key.slice(7) : key;
    if (!friendly) return;
    result = result.replace(new RegExp(`\\{\\{${i + 1}\\}\\}`, "g"), `{{${friendly}}}`);
  });
  return result;
}
...
export default function TemplateForm({ templateId }: TemplateFormProps) {
  ...
  const [currentStep, setCurrentStep] = useState<StepId>(isEdit ? "edit" : "setup");
  ...
  // Path stored in DB (e.g. "{projectId}/whatsapp-settings/uuid.jpg") — NOT a viewable URL.
  const [headerMediaPath, setHeaderMediaPath] = useState("");
  // Signed URL (or full URL) for showing the file in the browser. Not persisted.
  ...
  useEffect(() => {
    if (!isEdit || !activeProject?.id || !templateId) return;
    let cancelled = false;
    (async () => {
      ...
      try {
        ...
        if (cancelled) return;
        const t: ExistingTemplate = data.template;
        if (t.status !== "draft") {
          setReadOnly(true);
          toast.error("This template was already submitted and can no longer be edited.");
        }
        setName(t.name ?? "");
        setCategory((t.category as Category) ?? "utility");
        setLanguage(t.language ?? "en");
        const hf = (t.header_format ?? "text") as HeaderFormat;
        // DB default for non-text drafts is "text" if user picked "none" — distinguish by header content
        if (hf === "text" && !t.header) {
        ...
        setHeaderMediaPath(t.header_media_url ?? "");
        setHeaderMediaPreviewUrl(t.header_media_signed_url ?? "");
        setHeaderFileName(t.header_media_url ? (t.header_media_url.split("/").pop() ?? "") : "");
        // Convert numbered placeholders back to friendly names using mapping
        const friendlyBody = numberedBodyToFriendly(t.body ?? "", t.variable_field_mapping ?? []);
        setBody(friendlyBody);
        setFooter(t.footer ?? "");
        // Restore variable examples by friendly name from variables + mapping
        const vars = Array.isArray(t.variables) ? t.variables : [];
        const mapping = Array.isArray(t.variable_field_mapping) ? t.variable_field_mapping : [];
        const examples: Record<string, string> = {};
        mapping.forEach((key, i) => {
          if (!key) return;
          const friendly = key.startsWith("custom:") ? key.slice(7) : key;
          if (friendly) examples[friendly] = String(vars[i] ?? "");
        });
        setVarExamples(examples);
        // Map button types: server stores "quick_reply"/"url"/"phone_number". UI also has "copy_code" but
        // currently submitted as quick_reply, so no reverse mapping needed.
        const btns: TemplateButton[] = Array.isArray(t.buttons)
          ? t.buttons.map((b) => {
              const rawType = String(b.type ?? "quick_reply").toLowerCase();
              const type: ButtonType =
                rawType === "url" ? "url" :
                rawType === "phone_number" ? "phone_number" :
                "quick_reply";
              return {
                type,
                text: String(b.text ?? ""),
                url: type === "url" ? String(b.url ?? "") : undefined,
                phone_number: type === "phone_number" ? String(b.phone_number ?? "") : undefined,
              };
            })
          : [];
        setButtons(btns);
    ...
    return () => { cancelled = true; };
  }, [isEdit, activeProject?.id, templateId]);
  ...
  const handleHeaderUpload = async (file: File) => {
    ...
    try {
      ...
      setHeaderMediaPath(data.path);
      setHeaderMediaPreviewUrl(data.signed_url ?? "");
  ...
  const buildPayload = () => {
    ...
    return {
      ...
      header_media_url: !["none", "text"].includes(headerFormat) ? (headerMediaPath.trim() || null) : null,
  ...
  // Save (POST for new, PATCH for existing). Returns the persisted template id.
  // Drafts can be partial — only name is required to persist.
  const persist = async (): Promise<string | null> => {
    if (!activeProject?.id) return null;
    if (!name.trim()) { toast.error("Template name is required"); return null; }
    const payload = buildPayload();
    if (draftId) {
      const res = await fetch(`/api/projects/${activeProject.id}/whatsapp/templates/${draftId}`, {
        method: "PATCH",
    ...
    if (!id) throw new Error("No template ID returned");
    return id;
  };
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      if (!draftId) {
        setDraftId(id);
        // Switch URL to edit route so refresh keeps the draft open.
        router.replace(`/dashboard/whatsapp/templates/${id}/edit`);
      }
    } catch (err) {
  ...
  const handleSubmitForReview = async () => {
    ...
    if (!["none", "text"].includes(headerFormat) && !headerMediaPath.trim()) {
      toast.error(`Upload a ${headerFormat} sample for the header before submitting`);
      return;
    }
    setSaving(true);
    setSubmitting(true);
    try {
      ...
      const submitRes = await fetch(`/api/projects/${activeProject!.id}/whatsapp/templates/${id}/submit`, { method: "POST" });
    ...
    <div className="flex flex-col gap-6">
      ...
      {readOnly && (
        <div className="border border-amber-200 dark:border-amber-900 bg-amber-50 dark:bg-amber-950/30 px-4 py-3 text-sm">
          This template is no longer a draft and can&apos;t be edited. Create a new one to change content.
        </div>
      )}
        ...
        <div className="flex flex-col gap-6 max-w-3xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {CATEGORIES.map((cat) => {
              ...
              return (
                <button key={cat.value} type="button" onClick={() => setCategory(cat.value)} disabled={readOnly}
      ...
      {currentStep === "edit" && (
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6 items-start">
          ...
          {/* Left: Editor */}
          <fieldset className="flex flex-col gap-6 contents" disabled={readOnly}>
            <div>
              <h2 className="text-lg font-semibold">{isEdit ? "Edit draft" : "Edit template"}</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="tpl-name" value={name}
                  ...
                  placeholder="e.g. order_confirmation" maxLength={512} disabled={readOnly} />
                <p className="text-xs text-muted-foreground">{name.length}/512</p>
              </div>
              <div className="space-y-2">
                <Label>Language *</Label>
                <div className="relative" ref={langRef}>
                  <button type="button" onClick={() => setLangOpen(!langOpen)} disabled={readOnly}
            ...
            <div className="space-y-3 border border-gray-200 dark:border-gray-800 rounded p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {HEADER_FORMATS.map((fmt) => {
                  ...
                  return (
                    <button key={fmt.value} type="button" disabled={readOnly}
                      onClick={() => { setHeaderFormat(fmt.value); if (fmt.value === "none") { setHeaderText(""); setHeaderMediaPath(""); setHeaderMediaPreviewUrl(""); setHeaderFileName(""); } }}
              ...
              {headerFormat === "text" && (
                <div className="space-y-1">
                  <Input value={headerText} onChange={(e) => setHeaderText(e.target.value)} placeholder="Add a short line of text to the header" maxLength={60} disabled={readOnly} />
              ...
              {!["none", "text"].includes(headerFormat) && (
                <div className="space-y-2">
                  ...
                  {!headerFileName ? (
                    <button type="button" onClick={() => fileInputRef.current?.click()} disabled={headerMediaUploading || readOnly}
                    ...
                    <div className="space-y-2">
                      {/* Inline thumbnail / preview of the uploaded asset */}
                      {headerFormat === "image" && headerMediaPreviewUrl && (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={headerMediaPreviewUrl} alt={headerFileName} className="max-h-40 w-auto rounded border border-gray-200 dark:border-gray-800 object-contain" />
                      )}
                      {headerFormat === "video" && headerMediaPreviewUrl && (
                        <video src={headerMediaPreviewUrl} controls className="max-h-48 w-full rounded border border-gray-200 dark:border-gray-800" />
                      )}
                      <div className="flex items-center gap-2 border border-gray-200 dark:border-gray-800 rounded p-3">
                        ...
                        {headerMediaPreviewUrl && (
                          <a href={headerMediaPreviewUrl} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-foreground">
                            <ExternalLink className="h-4 w-4" />
                          </a>
                        )}
                        <button type="button" disabled={readOnly}
                          onClick={() => { setHeaderMediaPath(""); setHeaderMediaPreviewUrl(""); setHeaderFileName(""); }}
            ...
            <div className="space-y-3 border border-gray-200 dark:border-gray-800 rounded p-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {FIELD_CHIPS.map((chip) => (
                  <button key={chip.key} type="button" onClick={() => insertVariable(chip.key)} disabled={readOnly}
                ...
                {customFieldKeys.map((key) => (
                  <button key={key} type="button" onClick={() => insertVariable(key)} disabled={readOnly}
                    className="inline-flex items-center rounded border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/40 px-2 py-1 text-xs hover:bg-gray-100 dark:hover:bg-gray-800">
                    {key}
                  </button>
                ))}
              </div>
              <textarea id="tpl-body" ref={bodyRef} value={body} onChange={(e) => setBody(e.target.value)} disabled={readOnly}
                    ...
                    {namedVars.map((varName) => (
                      <div key={varName} className="flex items-center gap-2">
                        ...
                        <Input value={varExamples[varName] ?? ""} onChange={(e) => setVarExamples((prev) => ({ ...prev, [varName]: e.target.value }))}
                          placeholder={`Sample for ${varName}`} className="flex-1 h-8 text-sm" disabled={readOnly} />
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="space-y-2 border border-gray-200 dark:border-gray-800 rounded p-4">
              ...
              <Input id="tpl-footer" value={footer} onChange={(e) => setFooter(e.target.value)} placeholder="Add a short line of text to the footer" maxLength={60} disabled={readOnly} />
            ...
            <div className="space-y-3 border border-gray-200 dark:border-gray-800 rounded p-4">
                  ...
                  {buttons.map((btn, i) => (
                    <div key={i} className="border border-gray-200 dark:border-gray-800 rounded p-3 space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <button type="button" onClick={() => removeButton(i)} disabled={readOnly} className="text-muted-foreground hover:text-red-500"><X className="h-4 w-4" /></button>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        <div className="space-y-1">
                          <span className="text-xs text-muted-foreground">Button text</span>
                          <Input value={btn.text} onChange={(e) => updateButton(i, { text: e.target.value })} placeholder="Button text" maxLength={25} className="h-8 text-sm" disabled={readOnly} />
                        ...
                        {btn.type === "url" && (
                          <div className="space-y-1">
                            ...
                            <Input value={btn.url ?? ""} onChange={(e) => updateButton(i, { url: e.target.value })} placeholder="https://example.com" className="h-8 text-sm" disabled={readOnly} />
                          </div>
                        )}
                        {btn.type === "phone_number" && (
                          <div className="space-y-1">
                            <span className="text-xs text-muted-foreground">Phone number</span>
                            <Input value={btn.phone_number ?? ""} onChange={(e) => updateButton(i, { phone_number: e.target.value })} placeholder="+15551234567" className="h-8 text-sm" disabled={readOnly} />
                          </div>
                        )}
                        {btn.type === "copy_code" && (
                          <div className="space-y-1">
                            <span className="text-xs text-muted-foreground">Offer code sample</span>
                            <Input value={btn.offer_code ?? ""} onChange={(e) => updateButton(i, { offer_code: e.target.value })} placeholder="SAVE20" maxLength={20} className="h-8 text-sm" disabled={readOnly} />
                          </div>
                        )}
                      </div>
                    </div>
              ...
              {buttons.length < 10 && (
                <div className="relative" ref={btnDropdownRef}>
                  <Button type="button" variant="outline" size="sm" className="gap-1 text-xs" onClick={() => setBtnDropdownOpen(!btnDropdownOpen)} disabled={readOnly}>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-4 border-t border-gray-200 dark:border-gray-800">
              <Button variant="outline" onClick={() => setCurrentStep("setup")} className="gap-1" disabled={readOnly}>
                <ArrowLeft className="h-4 w-4" /> Back
              </Button>
              <div className="flex gap-2">
                <Button variant="outline" onClick={handleSaveDraft} disabled={saving || readOnly} className="gap-1 flex-1 sm:flex-none">
                  {saving && !submitting && <Loader2 className="h-4 w-4 animate-spin" />}
                  {draftId ? "Save draft" : "Save as draft"}
                </Button>
                <Button onClick={handleSubmitForReview} disabled={saving || readOnly} className="gap-1 flex-1 sm:flex-none">
                ...
                <div className="max-w-[280px]">
                  <div className="bg-white dark:bg-gray-900 rounded-lg overflow-hidden">
                    ...
                    {headerFormat === "image" && (
                      headerMediaPreviewUrl ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={headerMediaPreviewUrl} alt="Header" className="w-full max-h-44 object-cover" />
                      ) : (
                    ...
                    {headerFormat === "video" && (
                      headerMediaPreviewUrl ? (
                        <video src={headerMediaPreviewUrl} controls className="w-full max-h-44 bg-black" />
                      ) : (
                        <div className="bg-gray-200 dark:bg-gray-700 h-32 flex items-center justify-center">
